...
> 当前版本：`0.5.0`。第一版只监测 Tibo，不监测其他员工或官方账号。本项目与 OpenAI、X Corp.、Microsoft 或 Thibault Sottiaux 无隶属或背书关系。
...
“获取”和“过滤”是两层独立逻辑：X 页面获取模块先读取 Tibo 的全部可见帖子；随后本地分类模块使用 `lib/rules.js` 中的规则检查帖子文本。只有命中即将重置、可能重置、已完成重置、额度增加或规则变化的帖子才会进入最近 50 条事件历史并可能发送通知。否定表达（例如 `No reset is planned`）会被排除。整个过程不把帖子发送到服务器或 AI API。
