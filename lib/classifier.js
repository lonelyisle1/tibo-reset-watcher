...
export function classifyPost(rawText, settings = {}) {
  ...
  const hasAmbiguousResetSignal = matchesAny(text, AMBIGUOUS_RESET_SIGNAL_PATTERNS);
  const hasNonQuotaReset = matchesAny(text, NON_QUOTA_RESET_PATTERNS);
  ...
  if (hasNonQuotaReset && !hasContext) {
    return result(false, "unrelated", "none", "帖子中的 reset 明确指向设备、密码或其他非额度对象。", "无需操作。", supplemental);
  }
  ...
  if (hasAmbiguousResetSignal && !hasNegation) {
    return result(
      true,
      "possible_reset",
      "warning",
      "Tibo 使用了隐晦的 reset 暗示，可能与额度有关，但尚未明确说明范围或时间。",
      "先检查当前剩余额度并关注原帖后续；不要把这条暗示当作已经确认的全局重置。",
      [...supplemental, "ambiguous_reset_signal"]
    );
  }
