...
export const COMPLETED_RESET_PATTERNS = [
  ...
  /\breset\s+(?:has|had)\s+been\s+(?:propagated|applied|rolled\s+out)\b/i,
  /\b(?:brand\s+new|fresh|renewed)\s+(?:weekly\s+)?usage\b.{0,80}\b(?:chatgpt|codex|users?|plans?)\b/i,
  /\busage(?:\s+limits?)?\s+(?:has|have)\s+been\s+(?:reset|restored|refreshed|renewed|propagated)\b/i,
...
export const UPCOMING_RESET_PATTERNS = [
  ...
  /\b(?:we(?:'|’)re|we\s+are|i(?:'|’)m|i\s+am)\s+resett?ing\s+(?:everyone(?:'|’)s\s+|all\s+)?(?:codex\s+|chatgpt\s+work\s+|usage\s*(?:limits?)?|rate\s+limits?)\b/i,
  /\b(?:your|the|a|another|one)\b.{0,45}\breset\s+(?:will|should|is\s+going\s+to)\s+(?:land|arrive|roll\s+out)\b/i,
  /\breset\s+(?:will|should|is\s+going\s+to)\s+(?:land|arrive|roll\s+out)\b/i,
...
export const LIMIT_CHANGE_PATTERNS = [
  /\busage\s+limits?\b/i,
  /\busage\s+(?:allocation|allowance|pool|window)\b/i,
  /\b(?:weekly|normal|shared)\s+usage\b/i,
  /\brate\s+limits?\b/i,
